...
-----------------------------------------------------
 Copyright (c) 2022-2020 Cicada Media Group
=====================================================
...
HTML;
	...
	if( strpos ( $buffer, "dleplyrplayer" ) !== false ) {
		...
		if( strpos ( $buffer, ".m3u8" ) !== false ) {
			$load_more = "\$.getScript( dle_root + 'engine/classes/html5player/plyr.js');";
			$js_name = "hls.js"; 
		} else {
			$load_more = "";
			$js_name = "plyr.js"; 
		...
		<script>
			if (typeof DLEPlayer == "undefined") {
				...
				$.getScript( dle_root + 'engine/classes/html5player/{$js_name}', function() {
				  {$load_more} 
				});
