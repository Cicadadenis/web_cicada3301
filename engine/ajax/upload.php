...
if( $_POST['subaction'] == "deluploads" ) {
	...
	if( isset( $_POST['images'] ) ) {
		...
		foreach ( $_POST['images'] as $image ) {
			...
			foreach ( $temp_images as $dataimages ) {
	...
	$(document).on("click", ".uploadfile", function() {
		...
		if ( $(this).data('type') == "image" ) {
			...
			if ( mode == "video" || mode == "audio") {
				...
				$('#imgparam5').html('<label><input type="radio" name="filemode" value="1" checked="checked" />{$lang['media_upload_ip4']}</label>&nbsp;&nbsp;<label><input type="radio" name="filemode" value="0">{$lang['media_upload_ip5']}</label>');
