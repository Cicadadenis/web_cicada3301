...
function custom_print( $matches=array() ) {
	global $db, $is_logged, $member_id, $xf_inited, $cat_info, $config, $user_group, $category_id, $_TIME, $lang, $smartphone_detected, $dle_module, $allow_comments_ajax, $PHP_SELF, $news_date, $banners, $banner_in_news, $url_page, $user_query, $custom_news, $global_news_count, $remove_canonical, $custom_navigation, $row;
...
function create_keywords($story) {
	...
	$metatags['description'] = $story = trim(preg_replace('/\s+/u', ' ', $story));
...
function if_category_rating( $category ) {
	...
	foreach ( $category as $element ) {
		...
		if( $cat_info[$element]['rating_type'] > -1 ) {
...
function if_check($matches){
	...
	if (is_array($matches)) {
		...
		foreach ($if_array as $if_str) {
			...
			elseif( $field[0]=='date' OR $field[0]=='editdate' ) {
