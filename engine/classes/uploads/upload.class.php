...
class FileUploader {
    ...
    function __construct($area, $news_id, $author, $t_size, $t_seite, $make_thumb = true, $make_watermark = true, $m_size = 0, $m_seite = 0, $make_medium = false){        
		...
		if ( $this->area == "adminupload" ) {
			...
			if( $_REQUEST['userdir'] ) $userdir = cleanpath( $_REQUEST['userdir'] ). "/"; else $userdir = "";
			if( $_REQUEST['subdir'] ) $subdir = cleanpath( $_REQUEST['subdir'] ). "/"; else $subdir = "";
	...
	function FileUpload(){
		...
		if( !is_dir( ROOT_DIR . "/uploads/icons" ) ) {
			...
			@mkdir( ROOT_DIR . "/uploads/icons", 0777 );
